...
struct Logic
{
    Logic(const GroupVec g, int boost) : g_{g}, n_{g.size()} 
    { team_boost[0] = boost;
    ...
    GroupVec g_;
    size_t n_;
    std::array<int, 2> team_boost{0, 0};
    ...
    for (auto i = 0; i < g_.size(); ++i)
    {
        ...
        for (auto j = 0; j < g_.size(); ++j)
        {
            if (i != j && c.team_ != g_[j].team_ && afrom[j] < 0 && g_[j].units_ > 0)
            {
                ...
                //auto totdamage{getBoosted(c, g_[j])};
                cout << 
...
int main()
{
    ...
    while (true)
    {
        Logic logic{parsed, boost};
        //Logic logic{example, boost};

        auto ret{0}, prev{0}, next{0};
        do 
        {
            prev = logic.count();
            ret = logic.iter();
        ...
        while (ret == 0 && prev != next);
        cout << "boost=" << boost << " winner=" << ret << " rem=" << logic.count() << endl;
        if (ret == 1 || prev == next)
        {
            ++boost;
        }
